...
dnl                      for linking.
dnl     --with-opengl  - Give the root directory of the OpenGL implementation
dnl                      installation.
dnl
...
dnl opengl.m4,v 1.35 2008/01/01 15:29:21 patrickh Exp

dnl ---------------------------------------------------------------------------
dnl Determine if the target system has OpenGL (or Mesa3D) installed.  This
dnl adds command-line arguments --with-opengl and --enable-mesa.
dnl
dnl Usage:
...
dnl                           --with-opengl command-line argument.
...
[
   ...
   AC_ARG_WITH(opengl,
               [  --with-opengl=<PATH>    OpenGL installation directory   [default=$1]],
   ...
   if test "x$OGLROOT" != "xno" ; then
      ...
      if test "x$dpp_platform" = "xHP" ; then
         ...
         AC_CACHE_CHECK([for glEnable in -l${MESA}GL],
            [ac_cv_glEnable_available],
            [AC_TRY_LINK([#include <GL/gl.h>], [glEnable(GL_CULL_FACE)],
               [ac_cv_glEnable_available='yes'], [LIBS="$dpp_save_LIBS"])])
...
#include <GL/glu.h>],
         ...
         dnl For OpenGL under RedHat Linux 7.3, with nVidia drivers, we need
         dnl -pthread.
         if test "x$dpp_platform" = "xLinux" ; then
            optional_threading='-pthread'
         fi

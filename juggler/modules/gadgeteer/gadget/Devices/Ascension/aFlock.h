...
public:
	...
	aFlock(const char* const port = "/dev/ttyS0",
		const int& baud = 9600,
		const int& sync = 1,
		const int& block = 0,
		const int& numBrds = 3,
	...
	//: get the port used
	//  this will be a string in the form of the native OS descriptor <BR>
	//  ex: unix - "/dev/ttyd3", win32 - "COM3" linux - "/dev/ttyS0" <BR>
	const char* getPort() const;
	
	
        //: test to check the model of the flock
        //  this will generally return 6DFOB followed by 5 spaces
        void check_known();
	...
	inline const int&  getBlocking() const { return _blocking; }
        ...
        //: turn off hardware flow control on port
	void disable_hardware_flow_control();

	//: turn on hardware flow control on port

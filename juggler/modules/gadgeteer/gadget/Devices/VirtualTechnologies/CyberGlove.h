 ...
 *         joints[MIDDLE][DIP];
 */
class vjCyberGlove : public vjInput {

  public:	

    //vjCyberGlove (char *homedir, char *serport, int baud);
    /** vjConfigChunk Constructor.
     *
     *  Construct a glove, gets the homedirectory itself,
  ...
  //  void UpdateData();
    ...
    void UpdateData (); 
      ...
      /** returns a 2d array [MAX_SENSOR_GROUPS][MAX_GROUP_VALUES]
       * of the joint positions. You don't have to call this function
  ...
  private:
    ...
    matrix4x4 *xforms[5];
    vjGLOVE_DATA theData[3]; //[MAX_SENSOR_GROUPS][MAX_GROUP_VALUES];

...
bool vjKeyboardProxy::config(vjConfigChunk* chunk)
{
   ...
   {
      vjDEBUG(vjDBG_INPUT_MGR, vjDBG_CONFIG_LVL) << "vjKeyboardProxy::config: Could not find device: " << dev_name << std::endl << vjDEBUG_FLUSH;
      return false;
   }

   vjKeyboard* kb_dev = dynamic_cast<vjKeyboard*>(input_dev);
   if(NULL == kb_dev)
   {
      vjDEBUG(vjDBG_INPUT_MGR, vjDBG_CRITICAL_LVL) << "vjKeyboardProxy::config: Device was of wrong type: " << dev_name
   ...
   vjDEBUG_END(vjDBG_INPUT_MGR, vjDBG_STATE_LVL) << "   KeyboardProxy config()'ed" << std::endl << vjDEBUG_FLUSH;

   set(kb_dev);    // Set the proxy

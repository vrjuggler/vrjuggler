...
public class InputWindowPanel
   extends JPanel
   implements WizardSubPanel, ActionListener, FocusListener,
              ListSelectionListener
{
   ...
   private void processKeyboardChunk (ConfigChunk proxy_chunk, ConfigChunk window_chunk)
   {
      ...
      window_name = proxy_chunk.getProperty("device",0).getString();
      if( window_chunk == null )
      {
         window_chunk = results_db.get(window_name);
      }

      if ( window_chunk.getDescToken().equals("displayWindow") )
      {
         return; // we don't care about it.
      }

      x = window_chunk.getProperty("origin",0).getInt();
      y = window_chunk.getProperty("origin",1).getInt();
      width = window_chunk.getProperty("width",0).getInt();
      height = window_chunk.getProperty("height",0).getInt();
      ...
      iw.width = width;
      iw.height = height;
      iw.mouse_sensitivity = window_chunk.getProperty("msens",0).getFloat();
   ...
   public ConfigChunkDB getResultDB ()
   {
      if( results_dirty && (results_db != null) )
      {
         ...
         while( it.hasNext() )
         {
            ...
            ch.setProperty("origin", 0, new VarValue(iw.x));
            ch.setProperty("origin", 1, new VarValue(iw.y));

            ch.setProperty("width", 0, new VarValue(iw.width));
            ch.setProperty("height", 0, new VarValue(iw.height));

            ch.setProperty("msens", 0, new VarValue(iw.mouse_sensitivity));
   ...
   public void actionPerformed (ActionEvent e)
   {
      ...
      if( source == add_button )
      {
         ...
         proxy_chunk.setProperty("device", 0, new VarValue(window_name));
   ...
   {
      if ( null == current_editing_window )
      {
         return;
      }

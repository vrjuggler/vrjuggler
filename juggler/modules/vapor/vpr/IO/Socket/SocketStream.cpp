...
#include <IO/Socket/SocketStreamFactory.h>
...
// ----------------------------------------------------------------------------
SocketStream::SocketStream (const unsigned short port) {
    SocketStreamFactory fac;

    m_socket_stream_imp = fac.create(m_name, port, SocketTypes::INET);
...
// ----------------------------------------------------------------------------
SocketStream::SocketStream (const std::string& address,
                            const unsigned short port)
    : Socket(address)
{
    SocketStreamFactory fac;

    m_socket_stream_imp = fac.create(address, port, SocketTypes::INET);
    m_socket_imp        = m_socket_stream_imp;
}
...
SocketStream::SocketStream (const std::string& address) {
    ...
    m_socket_stream_imp = fac.create(host_addr, port, SocketTypes::INET);
    m_socket_imp        = m_socket_stream_imp;
}
...
// ----------------------------------------------------------------------------
SocketStream::SocketStream (const InetAddr& addr) {
    SocketStreamFactory fac;
...
// ----------------------------------------------------------------------------
bool
SocketStream::openServer (const int backlog) {
    bool retval;

...
public:
    ...
    // -----------------------------------------------------------------------
    ThreadWin32(thread_func_t func, void* arg = 0,
    ...
    // -----------------------------------------------------------------------
    ThreadWin32(BaseThreadFunctor* functorPtr,
                VPRThreadPriority priority = VPR_PRIORITY_NORMAL,
                VPRThreadScope scope = VPR_LOCAL_THREAD,
                VPRThreadState state = VPR_JOINABLE_THREAD,
                size_t stack_size = 0);
    ...
    inline int
    getprio (int* prio) {
        std::cerr << "vpr::ThreadWin32::getprio() not implemented yet!\n";
    ...
    inline int
    setprio (int prio) {
        std::cerr << "vpr::ThreadWin32::setprio() not implemented yet!\n";
    ...
    virtual int
    kill (int signum) {
        std::cerr << "vpr::ThreadWin32::kill() not implemented yet!\n";
...
public:
    ...
    //
    //! RETURNS: pointer - A pointer to this vpr::ThreadWin32 object's
    //+                    address.
    // -----------------------------------------------------------------------
    static BaseThread* self () {
        ...
        BaseThread* cur_thread = mThreadTable.getThread(thread_id);
...
private:
    static ThreadTable<DWORD> mThreadTable;
};

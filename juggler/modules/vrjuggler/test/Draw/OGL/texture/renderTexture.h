...
// destroy an OpenGL texture object currently associated with a Texture
// texObjectID: must be valid (not -1)
inline void tex::unbind( Texture& texture, int& texObjectID )
{
...
// is a Texture already an OpenGL texture object?
// return true if texObjectID is a valid OpenGL texture object...
inline bool tex::isBound( const Texture& texture, const int& texObjectID )
{
   if (texObjectID == -1)
      return false;
...
//              better performance
inline void tex::render( const Texture& texture, const int& texObjectID )
{
   const Image& image = texture.image();

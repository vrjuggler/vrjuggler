...
bool XMLConfigIOHandler::parseTextValues (Property* p, int& startval, char* text) {
    ...
    if (!retval) {
        vprDEBUG(vrjDBG_CONFIG,0) << clrOutNORM(clrRED, "ERROR:") <<
...
bool XMLConfigIOHandler::buildProperty (ConfigChunk* ch, const DOM_Node& doc, bool use_defaults) {
            ...
            while (retval && (child != 0)) {
                if (p->getType() == T_EMBEDDEDCHUNK) {
                    ...
                    case DOM_Node::COMMENT_NODE:
                        break;
                    default:
                        vprDEBUG(vrjDBG_CONFIG,2) << "XMLConfigIOHandler::"
                    ...
                    case DOM_Node::TEXT_NODE:
                        ...
                        break;
                    default:
                        vprDEBUG(vrjDBG_CONFIG,2) << "Warning: XMLConfigIOHandler::buildProperty(): Unexpected XML Node.\n" << vprDEBUG_FLUSH;
                        break;
                    }
                }
    ...
    case DOM_Node::COMMENT_NODE:
        break;
    default:
        vprDEBUG(vrjDBG_CONFIG,2) << "XMLConfigIOHandler::buildProperty:"
            " Unexpected DOM_Node.\n" << vprDEBUG_FLUSH;
    }
...
ConfigChunk* XMLConfigIOHandler::buildConfigChunk (const DOM_Node& doc, bool use_defaults) {
    ...
    case DOM_Node::COMMENT_NODE:
        break;
    default:
        vprDEBUG(vrjDBG_CONFIG,2) << "Warning: XMLConfigIOHandler::build"
    ...
    if (!retval) {
        ...
        if (ch) {
            vprDEBUG(vrjDBG_CONFIG,5) << "Rejecting ConfigChunk due to errors:\n"
                                    << *ch << vprDEBUG_FLUSH;
            // delete ch;
            ch = 0;
        }
...
/*virtual*/ bool XMLConfigIOHandler::readChunkDescDB (const std::string& filename, ChunkDescDB& db) {
    ...
    // ChunkFactory method, and the vprSingleton::instance() interface
...
ChunkDesc* XMLConfigIOHandler::buildChunkDesc (const DOM_Node& doc) {
    ...
    if (!retval) {
        ...
        // don't risk actually sending on some mangled thing.
        if (desc) {
            vprDEBUG(vrjDBG_CONFIG,5) << "Rejecting ChunkDesc due to errors:\n"
                                    << *desc << vprDEBUG_FLUSH;
            // delete desc;
            desc = 0;
        }
        ...
        if (!vjstrcasecmp (name, "PropertyDesc")) {
            ...
            for (int i = 0; i < attr_count; i++) {
                ...
                if (!vjstrcasecmp (child_name, "token"))
                    ...
                    vprDEBUG(vrjDBG_CONFIG,2) << "Warning: XMLConfigIOHandler"
            ...
            if (retval)
                ...
                vprDEBUG(vrjDBG_CONFIG,5) << "Rejecting PropertyDesc in " <<

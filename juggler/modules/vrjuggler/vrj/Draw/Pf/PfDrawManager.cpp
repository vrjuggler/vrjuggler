...
void vjPfDrawManager::config(vjConfigChunkDB*  chunkDB)
{
   ...
   vjDEBUG_BEGIN(0) << "------------- vjPfDrawManager::config ----------------" << endl;
   numPipes = sgiChunk->getProperty("numpipes");

   vjDEBUG(0) << "NumPipes: " << numPipes << endl;
   for (int i=0;i<numPipes;i++)
   {
      pipeStrs.push_back(sgiChunk->getProperty("xpipes", i));
      vjDEBUG(0) << "Pipe:" << i << ": " << pipeStrs[i] << endl;
   }
   ...
   if(perf_chunks->size() > 0)
   {
      ...
      if(head_file == NULL)
         vjDEBUG(0) << "vjPfDrawManager::config: simHeadModel not set." << endl;
      if(wand_file == NULL)
         vjDEBUG(0) << "vjPfDrawManager::config: simWandModel not set." << endl;
   ...
   vjDEBUG(0) << "Wand Model: " << mWandModel << endl;
   vjDEBUG_END(0) << "-----------------------------------------------------" << endl;
}
...
{
   vjDEBUG_BEGIN(0) << "vjPfDrawManager::initDrawing: Entering." << endl;
   ...
   {
      vjDEBUG(0) << "vjPfDrawManager::initDrawing: Opening Pipe." << endl;
      vjDEBUG(0) << "\tpipe:" << pipeNum << ": " << pipeStrs[pipeNum] << endl;
   ...
   {
      vjDEBUG(0) << "------- Opening new Display --------" << endl;
      
      pfDisp tempPfDisp;
      tempPfDisp.disp = (*dispIter);

      vjDEBUG(0) << "\tDisplay is:" << (void*)(*dispIter) << endl;
   ...
   vjDEBUG_END(0) << "vjPfDrawManager::initDrawing: Exiting." << endl;
}
...
{
   vjDEBUG(2) << "vjPfDrawManager::updateProjections: Entering." << endl << flush;
...
void vjPfDrawManager::updatePfProjection(pfChannel* chan, vjProjection* proj, bool simulator)
{
   ...
   vjDEBUG(7) << "Frustum: l:" << proj->frustum[vjFrustum::LEFT]
              ...
              << "   t: " << proj->frustum[vjFrustum::TOP] << endl << flush;
...
{
   vjDEBUG_BEGIN(0) << "-- DEBUG DUMP --------- vjPfDrawManager: " << (void*)this << " ------------" << endl;
   vjDEBUG(0)       << "app:" << (void*)app << endl;
   vjDEBUG(0)       << "scene:" << (void*)sceneRoot << endl;
   vjDEBUG(0)       << "Disps:" << disps.size() << endl;

   for (vector<pfDisp>::iterator i = disps.begin(); i != disps.end(); i++)
   {
      vjDEBUG(0) << "\n\tDisplay:" << (void*)(i->disp) << endl;
      vjDEBUG(0) << "\tpWin:" << (void*)(i->pWin) << endl;
      vjDEBUG(0) << "\t\tvis id:" << hex << i->pWin->getFBConfigId();
      vjDEBUG(0) << "\tchans: L:" << (void*)(i->chans[pfDisp::LEFT]) << "\tR:" << (void*)(i->chans[pfDisp::RIGHT]) << endl;  
   }

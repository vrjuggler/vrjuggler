...
Connect::Connect(Socket* s, const std::string& _name,
    ...
    vprDEBUG(vrjDBG_ENV_MGR,4) << "EM: Creating Connect to file or socket\n"
          << vprDEBUG_FLUSH;
    sock = s;
    mode = _mode;
...
void Connect::readControlLoop(void* nullParam) {
   vprDEBUG(vrjDBG_ENV_MGR,5) << "vjConnect " << name.c_str()
...
//          cout << " -done\n" << flush;
        ...
        while (!commands.empty()) {
            cmd = commands.front();
            commands.pop();
            vprDEBUG (vrjDBG_ENV_MGR, 5) << "calling EM command "
                                       << cmd->getName().c_str()
                                       <<vprDEBUG_FLUSH;
            cmd->call (*outstream);
            vprDEBUG (vrjDBG_ENV_MGR, 5) << " -- done.\n" << vprDEBUG_FLUSH;
            delete cmd;
        }
    ...
    vprDEBUG (vrjDBG_ENV_MGR,5) << "vjConnect " << name.c_str() << " ending write loop.\n" << vprDEBUG_FLUSH;
    //write_connect_thread = NULL;
    write_alive = false;
}
...
bool Connect::readCommand(std::istream& fin) {
    ...
    if (strncmp (buf, protocol_start_string, protocol_start_string_len)) {
        vprDEBUG (vrjDBG_ENV_MGR, 3) << "Connection: Invalid protocol identifier." << std::endl << vprDEBUG_FLUSH;
        do {
            if (!fin.getline (buf, buflen, '\n'))
                return false;
...
//      cout << "using protocol " << protocol_name << std::endl;
    ...
    if (communicator) {
        ...
        vprDEBUG (vrjDBG_ENV_MGR, 1) << "Connection: Couldn't find protocol handler for '" << protocol_name << "'\n" << vprDEBUG_FLUSH;
...
//      vprDEBUG(vrjDBG_ENV_MGR,4) << "vjConnect:: read: '" << rbuf
//                               << "'.\n" << vprDEBUG_FLUSH;
...
//              vprDEBUG(vrjDBG_ENV_MGR,4) << "vjConnect: Sending (requested) chunkdesc.\n" << vprDEBUG_FLUSH;
//              vprDEBUG(vrjDBG_ENV_MGR,5) << *db << std::endl << vprDEBUG_FLUSH;
//              sendDescDB (db);
//          }
...
//              vprDEBUG(vrjDBG_ENV_MGR,4) << "vjConnect: Sending (requested) chunkdb.\n" << vprDEBUG_FLUSH;
//              vprDEBUG(vrjDBG_ENV_MGR,5) << *db << std::endl << vprDEBUG_FLUSH;
//              sendChunkDB (db, true);
//          }
//          else {
...
//          //if (!strcasecmp (s, "all"))
//          //   chunkdb->removeAll()
//          vprDEBUG(vrjDBG_ENV_MGR,1) << "vjConnect:: Read: chunks: Started\n" << vprDEBUG_FLUSH;
...
//          //fin >> *newchunkdb;
//          vprDEBUG(vrjDBG_ENV_MGR,5) << *newchunkdb << std::endl << vprDEBUG_FLUSH;
//          vprDEBUG(vrjDBG_ENV_MGR,3) << "vjConnect:: Read: chunks: Completed\n" << vprDEBUG_FLUSH;
...
//          vprDEBUG(vrjDBG_ENV_MGR,3) << "vjConnect: Added chunks to ConfigManager pending list to add\n" << vprDEBUG_FLUSH;
//      }
...
//                  vprDEBUG(vrjDBG_ENV_MGR,3) << "EM Remove Descriptions disabled!\n" << vprDEBUG_FLUSH;
//              }
//          }
//          else if (!strcasecmp (s, chunks_TOKEN)) {
//              ConfigChunkDB* remove_chunk_db = new ConfigChunkDB();

//              vprDEBUG(vrjDBG_ENV_MGR,5) << "vjConnect: Remove: chunks: Starting...\n"  << vprDEBUG_FLUSH;

//              fin >> *remove_chunk_db;       // Read in the chunks to remove

//              vprDEBUG(vrjDBG_ENV_MGR,5) << *remove_chunk_db << std::endl
//                                       << vprDEBUG_FLUSH;
...
//              vprDEBUG(vrjDBG_ENV_MGR,3) << "vjConnect: Remove chunks added to ConfigManager pending list\n" << vprDEBUG_FLUSH;
//          }
//          else

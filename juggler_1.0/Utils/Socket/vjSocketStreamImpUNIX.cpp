...
#include <Utils/Socket/vjSocketStreamImpUNIX.h>
...
// ----------------------------------------------------------------------------
vjSocketStreamImpUNIX::vjSocketStreamImpUNIX (void)
    : vjSocketImpUNIX(), vjSocketStreamImp()
{
fprintf(stderr, "vjSocketStreamImpUNIX default constructor\n");
    /* Do nothing. */ ;
}
...
// variables of the object.
// ----------------------------------------------------------------------------
vjSocketStreamImpUNIX::vjSocketStreamImpUNIX (const std::string& address,
                                              const unsigned short port)
    : vjSocketImpUNIX(address, port, vjSocketTypes::INET,
...
fprintf(stderr, "vjSocketStreamImpUNIX(address, port) constructor\n");
...
vjSocketStreamImpUNIX::vjSocketStreamImpUNIX (const std::string& address,
    ...
    : vjSocketImpUNIX(address, port, domain, vjSocketTypes::STREAM),
      vjSocketStreamImp(address, port, domain)
{
    ...
    m_type = vjSocketTypes::STREAM;
fprintf(stderr, "vjSocketStreamImpUNIX(address, port, domain) constructor\n");
fprintf(stderr, "    Address: %s -> %s\n", address.c_str(), m_name.c_str());
fprintf(stderr, "    Port: %hu -> %hu\n", port, m_addr.getPort());
...
// ----------------------------------------------------------------------------
vjSocketStreamImpUNIX::~vjSocketStreamImpUNIX () {
    /* Do nothing. */ ;
}
...
// ----------------------------------------------------------------------------
bool
vjSocketStreamImpUNIX::listen (const int backlog) {
    bool retval;

    // Put the socket into listning mode.  If that fails, print an error and
    // return error status.
    if ( ::listen(m_handle->m_fdesc, backlog) == -1 ) {
        fprintf(stderr, "[vjSocketStreamImpUNIX] Cannot listen on socket: %s\n",
                strerror(errno));
        retval = false;
    }
...
vjSocketStreamImpUNIX::accept () {
    ...
    if ( accept_sock == -1 ) {
        fprintf(stderr,
                "[vjSocketStreamImpUNIX] Error while accepting incoming connection: %s\n",
                strerror(errno));
        new_sock = NULL;
    }
    // Otherwise, create a new vjSocketStreamImpUNIX object using what the
    // operating system gave us through accept(2).
    else {
        new_sock = new vjSocketStreamImpUNIX(accept_sock, addr);
    }
...
// ----------------------------------------------------------------------------
vjSocketStreamImpUNIX::vjSocketStreamImpUNIX (const int sock,
                                              vjInetAddr& host_addr)
    : vjSocketImpUNIX(), vjSocketStreamImp()
{
fprintf(stderr, "Protected vjSocketStreamImpUNIX constructor\n");
// XXX: Merge

 ...
 * --------------------------------------------------------------------------
 * NOTES:
 *    - This file (vjMutexNSPR.h) must be included by vjMutex.h, not the
 *      other way around.
 * --------------------------------------------------------------------------
 */

#ifndef _VJ_MUTEX_NSPR_H_
#define _VJ_MUTEX_NSPR_H_
...
public:
    // -----------------------------------------------------------------------
    //: Constructor for vjMutexNSPR class.
    //
    //! PRE: None.
        ...
        : mLocked(0)
    {
        // ----- Allocate the mutex ----- //
        mMutex = PR_NewLock();
        vjASSERT(mMutex != NULL);
    }

    // -----------------------------------------------------------------------
    //: Destructor for vjMutexNSPR class.
    //
    ...
    // Allow the vjCondNSPR class to access the private and protected members
    // of this class.  Specifically, direct access is needed to the mutex
    // variable.
    friend class vjCondNSPR;
...
protected:
    ...
    void operator= (const vjMutexNSPR &) {
        /* Do nothing. */ ;
    }

    vjMutexNSPR (const vjMutexNSPR &) {
        /* Do nothing. */ ;
    }
};

...
#ifndef _VJ_THREAD_NSPR_H_
#define _VJ_THREAD_NSPR_H_
//#pragma once
...
//!PUBLIC_API:
class vjThreadNSPR : public vjBaseThread
{
public:
    // -----------------------------------------------------------------------
...
//        return kill(SIGSTOP);
    ...
    // -----------------------------------------------------------------------
    inline virtual int
    getPrio (VJThreadPriority* prio) {
        *prio = nsprThreadPriorityToVJ(PR_GetThreadPriority(mThread));
...
private:
    ...
    // -----------------------------------------------------------------------
    // -----------------------------------------------------------------------
    PRThreadPriority vjThreadPriorityToNSPR(const VJThreadPriority priority);

    // -----------------------------------------------------------------------
    // -----------------------------------------------------------------------
    PRThreadScope vjThreadScopeToNSPR(const VJThreadScope scope);

    // -----------------------------------------------------------------------
    // -----------------------------------------------------------------------
    PRThreadState vjThreadStateToNSPR(const VJThreadState state);

    // -----------------------------------------------------------------------
    // -----------------------------------------------------------------------
    VJThreadPriority nsprThreadPriorityToVJ(const PRThreadPriority priority);

    // -----------------------------------------------------------------------
    // -----------------------------------------------------------------------
    VJThreadScope nsprThreadScopeToVJ(const PRThreadScope scope);

    // -----------------------------------------------------------------------
    // -----------------------------------------------------------------------
    VJThreadState nsprThreadStateToVJ(const PRThreadState state);
    ...
    static vjThreadTable<PRThread*>	mThreadTable;
};


#endif  /* _VJ_THREAD_NSPR_H_ */

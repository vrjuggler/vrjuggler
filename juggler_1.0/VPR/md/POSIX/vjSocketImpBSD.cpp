...
#include <VPR/md/POSIX/vjSocketImpBSD.h>
...
vjSocketImpBSD::open () {
...
#else
        fprintf(stderr,
                "[vjSocketImpBSD] WARNING: IPv6 not supported on this host!\n");
#endif
        break;
...
#else
        fprintf(stderr,
                "[vjSocketImpBSD] WARNING: Cannot use LINK sockets. domain %d\n",
                m_addr.getFamily());
#endif
        break;
      default:
        fprintf(stderr,
                "[vjSocketImpBSD] ERROR: Unknown socket domain value %d\n",
                m_addr.getFamily());
        break;
    }
    ...
    switch (m_type) {
      ...
      default:
        fprintf(stderr,
                "[vjSocketImpBSD] ERROR: Unknown socket type value %d\n",
                m_addr.getFamily());
        break;
    }
    ...
    if ( sock == -1 ) {
        fprintf(stderr, "[vjSocketImpBSD] Could not create socket (%s): %s\n",
...
vjSocketImpBSD::bind () {
    ...
    if ( status == -1 ) {
        fprintf(stderr, "[vjSocketImpBSD] Cannot bind socket to address: %s\n",
                strerror(errno));
        retval = false;
    }
    // Otherwise, return success.
...
vjSocketImpBSD::connect () {
    ...
    // error status.
    if ( status == -1 ) {
        fprintf(stderr, "[vjSocketImpBSD] Error connecting to %s: %s\n",
                m_name.c_str(), strerror(errno));
        retval = false;
    }
    // Otherwise, return success.
...
// ----------------------------------------------------------------------------
ssize_t
vjSocketImpBSD::recv (void* buffer, const size_t length, const int flags) {
...
// ----------------------------------------------------------------------------
ssize_t
vjSocketImpBSD::recv (unsigned char* buffer, const size_t length,
                      const int flags)
{
...
// ----------------------------------------------------------------------------
ssize_t
vjSocketImpBSD::recv (char* buffer, const size_t length, const int flags) {
    return recv((void*) buffer, length, flags);
}
...
// ----------------------------------------------------------------------------
ssize_t
vjSocketImpBSD::recv (std::string& buffer, const size_t length,
                      const int flags)
{
    ssize_t bytes;
    char* temp_buf;
...
// ----------------------------------------------------------------------------
ssize_t
vjSocketImpBSD::recv (std::vector<char>& buffer, const size_t length,
                      const int flags)
{
    ssize_t bytes;
    char* temp_buf;
...
// ----------------------------------------------------------------------------
ssize_t
vjSocketImpBSD::send (const void* buffer, const size_t length,
                      const int flags)
{
...
// ----------------------------------------------------------------------------
ssize_t
vjSocketImpBSD::send (const unsigned char* buffer, const size_t length,
                      const int flags)
{
...
// ----------------------------------------------------------------------------
ssize_t
vjSocketImpBSD::send (const char* buffer, const size_t length,
                      const int flags)
{
    return send((void*) buffer, length, flags);
}
...
// ----------------------------------------------------------------------------
ssize_t
vjSocketImpBSD::send (const std::string& buffer, const size_t length,
                      const int flags)
{
...
// ----------------------------------------------------------------------------
ssize_t
vjSocketImpBSD::send (const std::vector<char>& buffer, const size_t length,
                      const int flags)
{
    ssize_t bytes;
    char* temp_buf;
...
// defaults.
// ----------------------------------------------------------------------------
vjSocketImpBSD::vjSocketImpBSD ()
    : vjSocketImp(), m_handle(NULL)
{
fprintf(stderr, "vjSocketImpBSD default constructor\n");
    /* Do nothing. */ ;
}
...
// communications.
// ----------------------------------------------------------------------------
vjSocketImpBSD::vjSocketImpBSD (const std::string& address,
    ...
    : vjSocketImp(address, port, domain, type)
{
    ...
    m_type = type;
fprintf(stderr, "vjSocketImpBSD(address, port, domain, type) constructor\n");
...
vjSocketImpBSD::lookupAddress () {
    ...
    // If gethostbyname(3) failed, the address string may be an IP address.
        ...
        if ( addr == INADDR_NONE ) {
            fprintf(stderr,
                    "[vjSocketImpBSD] Could not find address for '%s': %s\n",
                    m_name.c_str(), strerror(errno));
            retval = false;
        }

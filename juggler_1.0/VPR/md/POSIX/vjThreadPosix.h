...
public:  // ---- Thread CREATION and SPAWNING -----
    ...
    // function.
    // -----------------------------------------------------------------------
    vjThreadPosix(vj_thread_func_t func, void* arg = 0,
    ...
    // -----------------------------------------------------------------------
    vjThreadPosix(vjBaseThreadFunctor* functorPtr,
                  VJThreadPriority priority = VJ_PRIORITY_NORMAL,
                  VJThreadScope scope = VJ_LOCAL_THREAD,
                  VJThreadState state = VJ_JOINABLE_THREAD,
                  size_t stack_size = 0);
    ...
    // -----------------------------------------------------------------------
    int spawn(vjBaseThreadFunctor* functorPtr,
              VJThreadPriority priority = VJ_PRIORITY_NORMAL,
              VJThreadScope scope = VJ_LOCAL_THREAD,
              VJThreadState state = VJ_JOINABLE_THREAD,
              size_t stack_size = 0);
...
private:
    ...
    // -----------------------------------------------------------------------
    // -----------------------------------------------------------------------
    int vjThreadPriorityToPOSIX(const VJThreadPriority priority);

    // -----------------------------------------------------------------------
    // -----------------------------------------------------------------------
    int vjThreadScopeToPOSIX(const VJThreadScope scope);

    // -----------------------------------------------------------------------
    // -----------------------------------------------------------------------
    int vjThreadStateToPOSIX(const VJThreadState state);

    // -----------------------------------------------------------------------
    // -----------------------------------------------------------------------
    VJThreadPriority posixThreadPriorityToVJ(const int priority);

    // -----------------------------------------------------------------------
    // -----------------------------------------------------------------------
    VJThreadScope posixThreadScopeToVJ(const int scope);

    // -----------------------------------------------------------------------
    // -----------------------------------------------------------------------
    VJThreadState posixThreadStateToVJ(const int state);

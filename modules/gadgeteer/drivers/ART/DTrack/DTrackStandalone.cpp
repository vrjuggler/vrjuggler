...
)
{
	...
	if(remote_host != NULL && remote_port > 0 && remote_port <= 65535){
		stat = d_remote.setAddress(remote_host, remote_port);
	
		if(!stat.success()){
			vprDEBUG(vprDBG_ALL, vprDBG_CRITICAL_LVL)
				<< "[DTrackStandalone] invalid remote address " << remote_host << ":" << remote_port
...
bool DTrackStandalone::cmd_send(int cmd, int val)
{
	...
	stat = d_udpsock->sendto(cmd_stream.str(), cmd_stream.str().length() + 1, d_remote, bytes, d_udptimeout);

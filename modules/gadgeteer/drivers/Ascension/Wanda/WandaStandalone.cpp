...
void WandaStandalone::start()
{
   ...
   if ( ! result.success() )
   {
      throw wanda::IOException("Failed to open serial port");
   }
   ...
   if ( bytes_read == 2 )
   {
      ...
      else
      {
         ...
         msg_stream << "Wanda communication startup failed.\n"
                    ...
                    << buffer[0] << buffer[1] << "\"";
         throw wanda::ProtocolException(msg_stream.str());
      }
   }
   else
   {
...
void WandaStandalone::sample()
{
   try
   {
      ...
      vpr::ReturnStatus result = mPort->read(&byte, 1, bytes_read, mTimeout);
...
void WandaStandalone::stop()
{
   ...
   {
      throw wanda::WandaException("Not running");
   }
}
                                       ...
                                       const vpr::Interval& timeout)
{
   ...
   vpr::Uint32 bytes_read(0);
   vpr::ReturnStatus result;
   result = mPort->read(buffer, length, bytes_read, timeout);
   ...
   {
      throw wanda::TimeoutException("Read operation timed out");
   }
   else
   {
      throw wanda::IOException("Read operation failed");
   }

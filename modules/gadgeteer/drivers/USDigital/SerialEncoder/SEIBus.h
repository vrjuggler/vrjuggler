...
private:
	...
	int searchSerialNumbers(unsigned int serialNumber,unsigned int mask){
		...
		for (bit = 0; bit <= 1; bit++) {				// try this bit low, then high 
			...
			if(waitForNotBusy(5)){						// busy stays active means match
				data[0] = WAKEUP;
				write(data,1);							// terminate command 
				if(mask == 0xFFFFFFFF){					// all bits matched 
				...
				else {									// bits matched, test next one 

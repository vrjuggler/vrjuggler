...
#ifndef _GADGET_DIGITAL_CLICK_EVENT_INTERFACE_H_
#define _GADGET_DIGITAL_CLICK_EVENT_INTERFACE_H_
...
/** \class DigitalClickEventInterface DigitalClickEventInterface.h gadget/Event/DigitalClickEventInterface.h
 *
 * A specialized digital event interface type for detecting click events from
 * a digital device. For a given instantiation, events for 1 to \c ClickCount
 * clicks are generated. The callback signature is the same as for
 ...
 *
 * \code
 * gadget::DigitalClickEventInterface<3> mClickIface;
 * \endcode
 *
 * it effectively declares interest in single, double-, and/or triple-click
 * events from a gadget::DigitalProxy. The callbacks are added as follows:
 *
 * \code
 * void singleClickCallback(const gadget::DigitalState::State)
 * {
 ...
 * }
 *
 * void tripleClickCallback(const gadget::DigitalState::State)
 * {
...
class DigitalClickEventInterface
...
{
public:
   typedef typename DigitalClickEventInterface::event_interface_ base_type;
    ...
    * @see setClickTime()
    */
   DigitalClickEventInterface(const unsigned long clickTime = 300)
      : mClickTime(clickTime)
   {
      /* Do nothing. */ ;
   }

   virtual ~DigitalClickEventInterface()
   {
      /* Do nothing. */ ;
   }
...
#endif /* _GADGET_DIGITAL_CLICK_EVENT_INTERFACE_H_ */

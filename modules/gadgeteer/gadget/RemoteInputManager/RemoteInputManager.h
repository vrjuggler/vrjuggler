...
protected:  
    ...
    enum {RIM_FALSE, RIM_TRUE, RIM_UNKNOWN} mRmtMgrChunkExists;  // needed to deteremine if we get our listening port
      ...
      if (port == mListenPort)
         ...
         if(mLongHostname.find(lowercase_host) != std::string::npos){
            ...
            if (getShortHostnameFromLong(lowercase_host) == mShortHostname)
               ...
               vprDEBUG(vrjDBG_INPUT_MGR, vprDBG_HEX_LVL) << lowercase_host << " is not equal to " << mShortHostname << std::endl << vprDEBUG_FLUSH;
         }
         else
            vprDEBUG(vrjDBG_INPUT_MGR, vprDBG_HEX_LVL) << lowercase_host << " is not in " << mLongHostname << std::endl << vprDEBUG_FLUSH;
   ...
   bool hostnameMatchesLocalHostname(const std::string& location){
      ...
      if(mLongHostname.find(test_hostname) != std::string::npos){
         ...
         if (getShortHostnameFromLong(test_hostname) == mShortHostname)
            ...
            vprDEBUG(vrjDBG_INPUT_MGR, vprDBG_HEX_LVL) << test_hostname << " is not equal to " << mShortHostname << std::endl << vprDEBUG_FLUSH;
      }
      else
         vprDEBUG(vrjDBG_INPUT_MGR, vprDBG_HEX_LVL) << test_hostname << " is not in " << mLongHostname << std::endl << vprDEBUG_FLUSH;
...
*/    
   ...
   bool listenWasInitialized(){ return mListenWasInitialized; }  // only true after config has been called
...
// ADDING AND REMOVING NET DEVICES    
   void acquireConfigMutex(){ mConfigMutex.acquire(); }
   void releaseConfigMutex(){ mConfigMutex.release(); }
   ...
   VJ_NETID_TYPE generateLocalId(){ return mLocalIdGen.generateNewId(); }

 ...
 * Juggler installation. Therefore, the root of the Gadgeteer installation is
...
extern "C" void __attribute ((constructor)) gadgetLibraryInit()
{
   Dl_info info;
   info.dli_fname = 0;
   const int result = dladdr(reinterpret_cast<const void*>(&gadgetLibraryInit),
                             &info);
   ...
   if ( 0 != result )
   {
      ...
      defined(VPR_OS_Linux) && defined(__x86_64__)
         ...
         // Start the search for the root of the Gadgeteer installation in the
         ...
         // of the Gadgeteer installation tree.
         ...
         setenv("GADGET_BASE_DIR", base_dir.native_directory_string().c_str(),
                0);
         setenv("GADGET_DATA_DIR", data_dir.native_directory_string().c_str(),
                0);
      }
      catch (fs::filesystem_error& ex)
      {

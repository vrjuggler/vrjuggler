...
#ifdef DOS
    char * sys_com_port[3] ={"com1","com2", "com3"};
#define OPENPARAMS O_RDWR
    ...
    */
#ifdef COHERENT
    char * sys_com_port[3] ={"/dev/com1l","/dev/com2l", "/dev/com3l"};
#define OPENPARAMS O_RDWR
#endif
   ...
   */
#ifdef AIX
    char * sys_com_port[3] ={"/dev/tty0","/dev/tty1","/dev/tty2"};
#define OPENPARAMS O_RDWR | O_NDELAY
   ...
   */
#ifdef SUNOS
    char * sys_com_port[3] ={"/dev/ttyha","/dev/ttyhb","/dev/ttyhc"};
#define OPENPARAMS O_RDWR | O_NDELAY
#endif
   ...
   */
#ifdef ULTRIX
    char * sys_com_port[3] ={"/dev/tty00","/dev/tty01","/dev/tty02"};
#define OPENPARAMS O_RDWR | O_NDELAY
#endif
   ...
   */
#ifdef IRIX
    char * sys_com_port[3] ={"/dev/ttyd1","/dev/ttyd2","/dev/ttyd3"};
#define OPENPARAMS O_RDWR
#endif
    ...
    */
    short comport = COM3;                   /* holds the comport # */

    /*
       Baud Rates for 6DFOBs
    */
    long baud = 38400L;                     /* holds the current baud rate */
    ...
    */
    ...
    short baudspeedbits = BAUDRATE_38400;            /* holds the current bit definition */
...
#define SETSERIALINT 0x14       /* INT 14h */
        ...
        */
        switch (comport)
        {
            ...
            case COM2:
                ...
                break;
	     case COM3:
                com_base = COM3BASE;                /* set the new I/O addr */
                break;
            default:
      ...
      */
      if (ioctl(comhandle,TCGETA,&tempcom_termio) >= 0)
      {
          ...
          */
#if defined THIS_LINE_NEEDS_TO_BE_CHANGED
	  /* ORs the previous baud bits with th new bits without clearing */
          /* the old bits. You need to clear the old ones out first. */
          tempcom_termio.c_cflag |= baudspeedbits;
#else
	  /* Clear the old baud bits out first. May not be _CBAUD */
          /* on all UNIX platforms */
          /*tempcom_termio.c_cflag &= ~CBAUD;*/
          tempcom_termio.c_cflag |= baudspeedbits;
#endif

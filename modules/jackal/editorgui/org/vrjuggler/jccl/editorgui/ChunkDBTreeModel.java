...
//      }
   ...
   protected void doReplaceChunk (String origName, ConfigChunk updatedChunk)
   {
      ...
      while( nodes.hasMoreElements() )
      {
         ...
         if( ni.isChunkNode() && ni.name.equals(origName) )
         {
            ni.name = updatedChunk.getName();
            nodeChanged (n);
         }
      }
   }
   ...
   protected void doRemoveChunk (ConfigChunk ch)
   {
      ...
      while( nodes.hasMoreElements() )
      {
         ...
         if( node.isLeaf() )
         {
            ...
            if ( ni.isChunkNode() && (ni.toString().equals(ch.getName())) )
            {
               v.add(node);
            }
         }
   ...
   protected void doAddChunk (ConfigChunk ch)
   {
      ...
      while( nodes.hasMoreElements() )
      {
         ...
         if ( ni.isDescNode() && ni.toString().equals(ch.getDescName()) )
         {
   ...
   final public class ReplaceChunkHandler implements Runnable
   {
      ...
      public ReplaceChunkHandler (String origName, ConfigChunk updatedChunk)
      {
         mOrigName = origName;
         mChunk    = updatedChunk;
      }
      ...
      public void run ()
      {
         doReplaceChunk(mOrigName, mChunk);
      }
   }
   ...
   public void configChunkAdded (ChunkDBEvent e)
   {
      ...
      {
         doAddChunk(e.getChunk());
      }
      else
      {
         try
         {
            SwingUtilities.invokeLater(new AddChunkHandler(e.getChunk()));
         }
         catch( Exception ex )
         {
         }
   ...
   public void configChunkReplaced (ChunkDBEvent e)
   {
      ...
      ConfigChunk chunk = e.getChunk();

      if( SwingUtilities.isEventDispatchThread() )
      {
         doReplaceChunk(e.getOrigChunkName(), chunk);
      }
      else
      {
         try
         {
            SwingUtilities.invokeLater(new ReplaceChunkHandler(e.getOrigChunkName(),
                                                               chunk));
         }
         catch( Exception ex )
         {
   ...
   public void configChunkRemoved (ChunkDBEvent e)
   {
      if( SwingUtilities.isEventDispatchThread() )
      {
         doRemoveChunk(e.getChunk());
      }
      else
      {
         try
         {
            SwingUtilities.invokeLater(new RemoveChunkHandler(e.getChunk()));
         }
         catch( Exception ex )
         {
         }

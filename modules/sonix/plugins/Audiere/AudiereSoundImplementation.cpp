...
XDL_EXPORT const char* getName() { return "Audiere"; }
XDL_EXPORT snx::ISoundImplementation* newPlugin() { return new snx::AudiereSoundImplementation; }
...
AudiereSoundImplementation::AudiereSoundImplementation() : snx::SoundImplementation(), mDev( NULL )
{
...
void AudiereSoundImplementation::setPitchBend( const std::string& alias, float amount )
{
...
int AudiereSoundImplementation::startAPI()
{
   if (mDev == NULL)
   {
      ...
      vprDEBUG(snxDBG, vprDBG_CONFIG_LVL) << clrOutNORM(clrYELLOW, "Audiere| NOTICE:") << " Audiere API started: [dev=" << (int)mDev<< "]\n" <<vprDEBUG_FLUSH;
   }
   else
   {

...
extern "C" void __attribute ((constructor)) snxLibraryInit()
{
   Dl_info info;
   info.dli_fname = 0;
   const int result = dladdr(reinterpret_cast<const void*>(&snxLibraryInit),
                             &info);
   ...
   if ( 0 != result )
   {
      ...
      defined(VPR_OS_Linux) && defined(__x86_64__)
         ...
         // of the Sonix installation tree.
         ...
         if ( found )
         {
            ...
            setenv("SNX_BASE_DIR", base_dir.native_directory_string().c_str(),
                   0);
            setenv("SNX_DATA_DIR", data_dir.native_directory_string().c_str(),
                   0);
         }
      }
      catch (fs::filesystem_error& ex)

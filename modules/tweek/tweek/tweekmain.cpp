...
extern "C" void __attribute ((constructor)) tweekLibraryInit()
{
   Dl_info info;
   info.dli_fname = 0;
   const int result = dladdr(reinterpret_cast<const void*>(&tweekLibraryInit),
                             &info);
   ...
   if ( 0 != result )
   {
      ...
      defined(VPR_OS_Linux) && defined(__x86_64__)
         ...
         // of the Tweek installation tree.
         ...
         setenv("TWEEK_BASE_DIR", base_dir.native_directory_string().c_str(),
                0);
         setenv("TWEEK_DATA_DIR", data_dir.native_directory_string().c_str(),
                0);
      }
      catch (fs::filesystem_error& ex)
      {
         std::cerr << "Automatic assignment of Tweek environment variables "
                   << "failed:\n" << ex.what() << std::endl;
      }
   }
}

...
public:
   ...
   void testThreadCreation()
   {
      ...
      std::vector<gfx::ThreadMemberFunctor<ThreadTest>*> functors(num_threads);
      std::vector<gfx::Thread*> threads(num_threads);

      for(int t=0;t<num_threads;t++)
      {
         functors[t] = new gfx::ThreadMemberFunctor<ThreadTest>(this,&ThreadTest::incCounter);

         // Spawns thread here
         threads[t] = new gfx::Thread(functors[t]);
      }
   ...
   void interactiveTestCPUGrind()
   {
      ...
      for(int t=0;t<num_threads;t++)
      {
         functors.push_back( new gfx::ThreadMemberFunctor<ThreadTest>(this,&ThreadTest::grindCPUWorker));
         threads.push_back( new gfx::Thread(functors[t]) );
      }
   ...
   void testThreadStackSize()
   {
      ...
      gfx::ThreadMemberFunctor<ThreadTest>* functor;
      gfx::Thread* the_thread;

      functor = new gfx::ThreadMemberFunctor<ThreadTest>(this,&ThreadTest::recurseConsumeResources, &arg);

      the_thread = new gfx::Thread(functor, stack_size);

...
public:
   ...
   void accelerate(const Vec3& accel);
...
protected:
   ...
   void setRotationalAcceleration( const Matrix& mat )
   { mRotationalAcceleration = mat;}

   // Rotate the environment
   void scaled_rotate(Matrix rot_mat);

   // util func... used to convert gravity from meters...
   inline void meters2feet( const Vec3& gravityInMeters, Vec3& gravityInFeet )
   {
...
private:
   Vec3 mVelocity;
   Vec3 mVelocityFromGravityAccumulator;
   float    mMaxVelocity;
   Matrix mRotationalAcceleration;
   float    mDamping;
   float    mAcceleration;

   // digitalInterfaces
   std::vector<DigitalInterface*> mActionButtons;
   PosInterface mNavWand;
...
void velocityNav::setActionButtons(std::vector<std::string> action_btn_names)
{
   ...
   {
      mActionButtons.push_back(new DigitalInterface());
   }
...
void velocityNav::updateInteraction()
{
   ...
   { accelerate(Vec3(0, 0, -mAcceleration)); }
   ...
   // Rotating
   Matrix rot_mat = *(mNavWand->getData());
   //std::cout<<"1: "<<rot_mat<<"\n"<<std::flush;
...
void velocityNav::update()
{
   ...
   Vec3         trackerZaxis(0.0f, 0.0f, 1.0f);
   Vec3         trackerXaxis(1.0f, 0.0f, 0.0f);
   Vec3         trackerYaxis(0.0f, 1.0f, 0.0f);
   const Vec3   gravity_meters_per_second( 0.0f, -9.8f, 0.0f ); //9.8 m/s (METERS)
   Vec3         gravity( 0.0f, -9.8f, 0.0f ); // to be set by switch (mUnits == METERS)
   ...
   Vec3 distanceToMove = velocityAccumulator * stopWatch.timeInstant;
   ...
   Vec3   total_correction;          // The total correction on the movement (in modelspace)
...
void velocityNav::scaled_rotate(Matrix rot_mat)
{
...
void velocityNav::accelerate(const Vec3& accel)
{

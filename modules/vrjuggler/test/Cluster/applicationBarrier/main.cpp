...
//----------------------------------------
// Test Application Barrier VR Juggler Program
//
// main.cpp
//----------------------------------------
...
{
   return new applicationBarrierApp(kernel);
}
#else
int main(int argc, char* argv[])
{
   ...
   applicationBarrierApp* application = new applicationBarrierApp(kernel);   // Declare an instance of the app

   // Create options for the barrierTest application.
   po::options_description barrier_desc("Barrier App Options");
   po::options_description& general_desc = kernel->getGeneralOptions();
   po::options_description& cluster_desc = kernel->getClusterOptions();
   barrier_desc.add(general_desc).add(cluster_desc);

   barrier_desc.add_options()
   ...
   po::parsed_options parsed = parser.options(barrier_desc).positional(pos_desc).allow_unregistered().run();
   ...
   if(vm.count("help"))
   {
      std::cout << barrier_desc << std::endl;
      return 0;
   }
   ...
   if ( 0 == vm.count("jconf") && 0 == vm.count("vrjslave"))
   {
      ...
      error_output << barrier_desc << std::endl;

...
//   - If WndProc can find D3dWindowWin32
//     - Send message to D3dWindowWin32
...
public:
   ...
   friend std::ostream& operator<<(std::ostream& out, D3dWindow& win);
...
protected:
   ...
   static void addWindow(HWND handle, D3dWindow* glWin);

   /** Removes a window from the map. */
   static void removeWindow(HWND handle);

   /** Gets the glWin we are dealing with. */
   static D3dWindow* getD3dWin(HWND handle);

   static std::map<HWND, D3dWindow*> mD3dWinMap;
...
// ostream& operator<<(ostream& out, D3dWindow& win);

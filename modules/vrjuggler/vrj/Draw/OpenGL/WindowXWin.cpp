...
bool WindowXWin::open()
{
   // Attempts to open the window and create the OpenGL context.  Does nothing
   // if the window is already open (and returns true).
   // Returns true for success; false for failure.
   // The newly opened window will be set as the calling proccess' current
   // OpenGL context.
   ...
   vprDEBUG(vrjDBG_DRAW_MGR, vprDBG_STATE_LVL)
      << "[vrj::opengl::WindowXWin::open()] Open window\n" << vprDEBUG_FLUSH;
   ...
   if ( mWindowWidth == -1 )
   {
      vprDEBUG(vprDBG_ERROR, vprDBG_CRITICAL_LVL)
         ...
         << ": [vrj::opengl::WindowXWin::open()] Window has not been "
         << "configured!\n" << vprDEBUG_FLUSH;
      return false;
   }
   ...
   if ( ! (mXDisplay = ::XOpenDisplay(mXDisplayName.c_str())) )
   {
      vprDEBUG(vprDBG_ERROR, vprDBG_CRITICAL_LVL)
         << clrOutNORM(clrRED, "ERROR")
         << ": [vrj::opengl::WindowXWin::open()] Unable to open display '"
         << mXDisplayName << "'.\n" << vprDEBUG_FLUSH;
      return false;
   }
   ...
   try
   {
      ...
      if ( w_attrib.colormap == 0 )
      {
         vprDEBUG(vprDBG_ERROR, vprDBG_CRITICAL_LVL)
            << clrOutNORM(clrRED, "ERROR")
            << ": [vrj::opengl::WindowXWin::open()] XCreateColorMap failed "
            << "on '" << mXDisplayName << "'.\n" << vprDEBUG_FLUSH;
         throw glwinx_OpenFailureException();
      }
...
//      event_mask = ExposureMask | StructureNotifyMask;   // Don't request buttons or keys since that will be handled elsewhere
      ...
      if ( 0 == mXWindow )
      {
         vprDEBUG(vrjDBG_DRAW_MGR, vprDBG_CRITICAL_LVL)
            << clrOutNORM(clrRED, "ERROR")
            << ": [vrj::opengl::WindowXWin::open()] Couldn't create window "
            << "for " << mXDisplayName << std::endl << vprDEBUG_FLUSH;
         throw glwinx_OpenFailureException();
      }
      ...
      if ( ! mHasBorder )
      {
         vprDEBUG(vrjDBG_DRAW_MGR, vprDBG_HVERB_LVL)
            << "[vrj::opengl::WindowXWin::open()] Attempting to make window "
         ...
         if ( MotifHints == None )
         {
            vprDEBUG(vrjDBG_DRAW_MGR, vprDBG_CRITICAL_LVL)
               << clrOutNORM(clrRED, "ERROR")
               << ": [vrj::opengl::WindowXWin::open()] Could not get X atom "
               << "for _MOTIF_WM_HINTS." << std::endl << vprDEBUG_FLUSH;
         }
         else
         {
            MotifWmHints hints;
      ...
      if (None == net_wm_state)
      {
         vprDEBUG(vrjDBG_DRAW_MGR, vprDBG_CRITICAL_LVL)
            << clrOutNORM(clrRED, "ERROR")
            << ": [vrj::opengl::WindowXWin::open()] Could not get X atom for "
            << "_NET_WM_STATE." << std::endl << vprDEBUG_FLUSH;
      }
      else
      {
         ...
         if (mIsFullScreen)
         {
            if (None == fullscreen_hint)
            {
               vprDEBUG(vrjDBG_DRAW_MGR, vprDBG_CRITICAL_LVL)
                  << clrOutNORM(clrRED, "ERROR")
                  << ": [vrj::opengl::WindowXWin::open()] Could not get X "
                  << "atom for _NET_WM_STATE_FULLSCREEN." << std::endl
                  << vprDEBUG_FLUSH;
            }
            else
            {
         ...
         if (mAlwaysOnTop)
         {
            if (None == above_hint || None == always_hint)
            {
               vprDEBUG(vrjDBG_DRAW_MGR, vprDBG_CRITICAL_LVL)
                  << clrOutNORM(clrRED, "ERROR")
                  << ": [vrj::opengl::WindowXWin::open()] Could not get X "
                  << "atoms for _NET_WM_STATE_ABOVE and "
         ...
         if (mMaximized)
         {
            if (None == maxh_hint || None == maxv_hint)
            {
               vprDEBUG(vrjDBG_DRAW_MGR, vprDBG_CRITICAL_LVL)
                  << clrOutNORM(clrRED, "ERROR")
                  << ": [vrj::opengl::WindowXWin::open()] Could not get X "
                  << "atoms for _NET_WM_STATE_MAXIMIZED_(HORZ/VERT).\n"
                  << vprDEBUG_FLUSH;
            }
            else
            {
         ...
         if (mModal)
         {
            if (None == model_hint)
            {
               vprDEBUG(vrjDBG_DRAW_MGR, vprDBG_CRITICAL_LVL)
                  << clrOutNORM(clrRED, "ERROR")
                  << ": [vrj::opengl::WindowXWin::open()] Could not get X "
                  << "atom for _NET_WM_STATE_MODAL." << std::endl
                  << vprDEBUG_FLUSH;
            }
            else
            {
         ...
         */
         ...
         XChangeProperty(mXDisplay, mXWindow, net_wm_state, XA_ATOM,
                         32, PropModeReplace, (unsigned char*) net_winstates,
                         curr_winstate);
      }
      ...
      vprDEBUG(vrjDBG_DRAW_MGR, vprDBG_VERB_LVL)
         << "[vrj::opengl::WindowXWin::open()] Done mapping window.\n"
         << vprDEBUG_FLUSH;
   ...
   catch (glwinx_OpenFailureException& openFailed)
   {
      ...
      // vrj::opengl::WindowXWin, and will deallocate all the stuff we might
...
void WindowXWin::configWindow(vrj::DisplayPtr disp)
{
   ...
   // NOTE: ConfigElements return the default value for a property if a value
   //       is not present. So if a pipe string is not specified for this pipe
   //       then it gets the default value of "-1".
   ...
   vprDEBUG(vrjDBG_DRAW_MGR, vprDBG_VERB_LVL)
      << "[vrj::opengl::WindowXWin::config] Display name is '"
      << mXDisplayName << "'" << std::endl << vprDEBUG_FLUSH;
}

void WindowXWin::checkEvents()
{
...
int WindowXWin::eventIsMapNotify(::Display*, XEvent* e, XPointer window)
{
   return e->type == MapNotify && e->xmap.window == (::Window) window;
}

void WindowXWin::createEmptyCursor(::Display* display, ::Window root)
{
   Pixmap cursormask;
   XGCValues xgc;
   GC gc;
...

{
   ...
   if ( use_create_context_attribs )  // We want to try to use the ARB function
   {
      ...
      if ( mExtensions.hasCreateContextARB() )
      {
         ...
         {
            attribList[6] = GLX_CONTEXT_PROFILE_MASK_ARB;

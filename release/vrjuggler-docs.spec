...
rm -f %{buildroot}/tweek/1.0/beanlist.xsd
# Ignore the Juggler build system documentation.
rm -rf %{buildroot}/docs
mkdir -p %{buildroot}%{_docdir}
mv %{buildroot}/vapor/%{vpr_version} %{buildroot}%{_docdir}/vpr-%{vpr_version}
mv %{buildroot}/tweek/%{tweek_version} %{buildroot}%{_docdir}/tweek-%{tweek_version}
mv %{buildroot}/jccl/%{jccl_version} %{buildroot}%{_docdir}/jccl-%{jccl_version}
mv %{buildroot}/sonix/%{sonix_version} %{buildroot}%{_docdir}/sonix-%{sonix_version}
mv %{buildroot}/gadgeteer/%{gadgeteer_version} %{buildroot}%{_docdir}/gadgeteer-%{gadgeteer_version}
mv %{buildroot}/vrjuggler/%{vrjuggler_version} %{buildroot}%{_docdir}/vrjuggler-%{vrjuggler_version}
